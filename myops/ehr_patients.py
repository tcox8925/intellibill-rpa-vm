...
Self-contained script that mirrors the tebra_rpa.py flow:
  ...
  4.  Scrape the All Patients MuiDataGrid → upsert into wpo.ehr_patients
  5.  For un-scraped patients: click patient → Account → Insurance tab →
      Edit primary case → scrape Policy #1 + #2 (plan name + policy number)

The table  wpo.ehr_patients  is EHR-agnostic — uses  ehr_name  column
...
EMAIL = "jpoorna@834labs.com"
PASSWORD = "Greenlantern$17)#93"

EHR_NAME = "Tebra"  # stored in ehr_name column

KEY_VAULT_NAME = "keyvault-834analytics"
KEY_VAULT_URL = f"https://{KEY_VAULT_NAME}.vault.azure.net/"

POSTGRES_CONFIG_PCH = {
    "host": "pch-db-dev001.postgres.database.azure.com",
    "database": "postgres",
    "user": "834data_syndb_adm",
}

POSTGRES_CONFIG_MYOPS = {
    "host": "myopsprod.postgres.database.azure.com",
    "database": "postgres",
    "user": "834data_syndb_adm",
}

CLIENT_ID_KEY = "SynapseAccessClientId"
CLIENT_SECRET_KEY = "SynapseAccessSecret"
TENANT_ID_KEY = "TenantId"

PATIENTS_TABLE = "wpo.ehr_patients"
...
def get_pch_connection():
    """Connect to pch-db-dev001 — EHR data tables."""
...
CREATE TABLE IF NOT EXISTS wpo.ehr_patients (
...
ON wpo.ehr_patients (ehr_name, patient_id, entity, sub_entity)
...
def scrape_practice_patients(
    ...
    For one practice (already selected in Tebra UI):
      ...
      B.  Scrape All Patients grid → upsert into wpo.ehr_patients
...
def run_patient_insurance_rpa(
    ...
    Flow:
      ...
      3. For each practice:
         ...
         b. Analytics → Patients → scrape grid → upsert wpo.ehr_patients
