...
from app.db.session_pch import get_db
...
router = APIRouter(tags=["PCH-CRM NEW ROUTES"])
...
@router.get("/pch-crm-new/providers")
async def get_providers(
    ...
    if state:
        ...
        state_clean = func.upper(func.trim(Pch_Provider_Info.state))

        if state_filter == "OTHER":
            filters.append(
                or_(
                    ~state_clean.in_(VALID_US_STATES),
                    Pch_Provider_Info.state.is_(None)
                )
            )
        else:
            filters.append(state_clean == state_filter)
    ...
    if zip:
        ...
        if clean_zip:
            zip_conditions = [
                Pch_Provider_Info.zip.ilike(f"{z}%")  # starts with
                for z in clean_zip
            ]
            filters.append(or_(*zip_conditions))
    ...
    if status:
        ...
        if clean_status:
            ...
            if "All" in clean_status:
                filters.append(
                    Pch_Provider_Info.status.in_(["Prospect","Active","Compliance", "Deactivated"])
                )
                pass
    ...
    if status and "Lead" in status:
        total_count = "68299"  # or None
    else:
        total_count = db.execute(
            select(func.count(Pch_Provider_Info.pk_id))
            .where(and_(*filters))
        ).scalar()
...
@router.post("/pch-crm-new/regulatory-validation-check")
def regulatory_validation_check(
    ...
    ranked = (
        db.query(
            ...
            .over(
                ...
                order_by=Pch_Regulatory_Validation.date_time.desc(),
            )
            .label("rn"),
        )
    ...
    result = (
        db.query(
            ranked.c.provider_id,
            func.bool_and(~ranked.c.status.in_(FAIL_STATES)).label("all_pass"),
        )
        .filter(ranked.c.rn == 1)
        .group_by(ranked.c.provider_id)
        .all()
    )

    return [
        {"id": r.provider_id, "value": "true" if r.all_pass else "false"}
        for r in result
    ]


@router.get("/pch-crm-new/state-filter")
async def get_state_filters(
    entity_id: str, sub_entity_id: str, status: str, db: Session = Depends(get_db)
):
    filters = [
        Pch_Provider_Info.group_id == sub_entity_id,
        Pch_Provider_Info.company_id == entity_id,
    ...
    if status is None:
        ...
        # exclude 'All'
        filters.append(Pch_Provider_Info.status != "All")
    else:
        # filter by the specific status
        filters.append(Pch_Provider_Info.status == status)
    ...
    # Sort alphabetically
    valid_states = sorted(valid_states, key=lambda x: x["value"])
...
@router.get("/pch-crm-new/supporting-filters")
async def get_supporting_filters(
    ...
    try:
        # Normalize state
        state_filter = None if not state else state.strip().upper()
        ...
        if state_filter:
            ...
            if state_filter == "OTHER":
                base_filters.append(
                    or_(
                        ~state_clean.in_(VALID_US_STATES),
                        Pch_Provider_Info.status != 'Lead',
                        Pch_Provider_Info.state.is_(None)
                    )
                )
            else:
                base_filters.append(state_clean == state_filter)

        if status_filter:
            if status_filter == "Compliance":
                base_filters.append(
                    Pch_Provider_Info.status.in_(["Compliance", "Deactivated"])
                )
            else:
        ...
        # Query unique statuses
        zip_5 = func.substring(func.trim(Pch_Provider_Info.zip), 1, 5).label("zip_5")
        zips = (
            db.query(zip_5)
            .filter(
                *base_filters,
                Pch_Provider_Info.zip.isnot(None),
                func.length(func.trim(Pch_Provider_Info.zip)) >= 5
            )
            .distinct()
            .order_by(zip_5)
            .all()
...
@router.get("/pch-crm-new/npi-search")
async def search_npi(
    ...
    try:
        ...
        state = None if not state else state.strip().upper()
        ...
        if state:
            ...
            if state == "OTHER":
                query = query.filter(~state_clean.in_(VALID_US_STATES))
            else:
                query = query.filter(state_clean == state)
        if owner:
            owner_conditions = []
            if "null" in owner:
...
@router.get("/pch-crm-new", response_model=PchProviderInfoSchema)
async def get_provider_details(
    ...
    query = (
        db.query(Pch_Provider_Info)
        .filter(
            Pch_Provider_Info.txn_id == txn_id,
            Pch_Provider_Info.company_id == entity_id,
            Pch_Provider_Info.group_id == sub_entity_id
        )
        .first()
...
@router.patch("/pch-crm-new", response_model=PchProviderInfoSchema)
async def update_provider_details(
    ...
    existing_provider = (
        db.query(Pch_Provider_Info)
        .filter(Pch_Provider_Info.npi == provider.npi)
        .first()
    )

    if not existing_provider:

        # Step 1: Create new provider
        provider_data = provider.model_dump(exclude_none=True, exclude_unset=True)
        provider_data.pop("updated_on", None)
        existing_provider = Pch_Provider_Info(**provider_data)
        existing_provider.pk_id = str(uuid())
        # raise HTTPException(
        #     status_code=status.HTTP_404_NOT_FOUND,
        ...
        # Step 2: Fetch NPI addresses
        npi_addresses = get_npi_addresses(provider.npi, db)
        seen_addresses = set()
        for i, addr in enumerate(npi_addresses[:2]):  # only first 2 addresses
            key = (addr["line1"], addr["city"], addr["state"], addr["zip"])
            ...
            db.add(
                Pch_Provider_Address(
                    pk_id=str(uuid()),
                    provider_id=existing_provider.txn_id,
                    provider_npi=provider.npi,
                    address_type_id=(
                        "9ecde8f0-d7c2-4284-a49d-2091c4c9e983"
                        if i == 0
                        else "f9d623cc-ce32-425d-abe1-b0982e329eaf"
                    ),
                    line1=addr["line1"],
        ...
        # Step 3: Fetch NPI phones
        npi_phones = get_npi_phone_text(provider.npi, db)
        seen_phones = set()
        for i, phone in enumerate(npi_phones):
            if phone["value"] in seen_phones:
                continue
            ...
            db.add(
                Pch_Provider_Communication(
                    ...
                    provider_id=existing_provider.txn_id,
                    provider_npi=provider.npi,
                    communication_id="b7ca1fc4-3b9b-42a2-b8d3-a602525946c4",
                    value=phone["value"],
                    text_opt=True,
                    dnd=True,
                    ai_pre_recording=True,
        ...
        # Commit all addresses and phones at once
        db.commit()
        db.refresh(existing_provider)

    # Step 4: Update existing provider fields
    if user.get("email"):
        existing_provider.touch_user = user["email"]

    update_data = provider.model_dump(
        exclude_none=True, exclude_unset=True, exclude={"touch_user"}
    )
    for key, value in update_data.items():
        setattr(existing_provider, key, value)

    # Step 5: Notify external service if status is Prospect/Active
    ...
    # Step 6: Handle CAQH validation (if caqh_number is provided)
    if provider.caqh_number:
        ...
        if status_normalized != "submitted":
            msg = (
                response_data.get("message") or f"Unexpected CAQH status: {raw_status}"
            )
            # raise HTTPException(
            #     status_code=status.HTTP_200_OK,
            #     detail=msg
...
@router.get("/pch-crm-new/provider-address-types")
def get_provider_address_types(db: Session = Depends(get_db)):
...
@router.get("/pch-crm-new/provider-address/{provider_id}")
def get_provider_addresses(provider_id: str, db: Session = Depends(get_db)):
    addresses = (
        db.query(
            Pch_Provider_Address.pk_id,
            Pch_Provider_Address.address_type_id,
            AddressType.type,
            Pch_Provider_Address.line1,
            Pch_Provider_Address.line2,
            Pch_Provider_Address.street,
            Pch_Provider_Address.city,
            Pch_Provider_Address.state,
            Pch_Provider_Address.county,
            Pch_Provider_Address.zip,
            Pch_Provider_Address.primary
        )
        .join(AddressType, AddressType.pk_id == Pch_Provider_Address.address_type_id)
        .filter(Pch_Provider_Address.provider_id == provider_id)
        .all()
    )
...
@router.post("/pch-crm-new/provider-address")
def create_provider_address(payload: ProviderAddressCreateSchema, db: Session = Depends(get_db)):
    provider = db.query(Pch_Provider_Info).filter(Pch_Provider_Info.txn_id == payload.provider_id).first()
    if not provider:
        raise HTTPException(status_code=404, detail="Provider not found")

    # Use provider.npi if payload.provider_npi not provided
    npi = payload.provider_npi or provider.npi

    existing_address_count = db.query(Pch_Provider_Address).filter(
        Pch_Provider_Address.provider_id == payload.provider_id
    ).count()
    ...
    if payload.primary:
        db.query(Pch_Provider_Address).filter(
            Pch_Provider_Address.provider_id == payload.provider_id
        ).update({"primary": False})

    new_address = Pch_Provider_Address(
...
@router.patch("/pch-crm-new/provider-address")
def update_provider_address(payload: ProviderAddressUpdateSchema, db: Session = Depends(get_db)):
    address_record = db.query(Pch_Provider_Address).filter(
        Pch_Provider_Address.pk_id == payload.pk_id,
        Pch_Provider_Address.provider_id == payload.provider_id
    ).first()
...
@router.delete("/pch-crm-new/provider-address/{pk_id}")
def delete_provider_address(pk_id: str, db: Session = Depends(get_db)):
    address_record = db.query(Pch_Provider_Address).filter(
        Pch_Provider_Address.pk_id == pk_id
    ).first()

    if not address_record:
        raise HTTPException(status_code=404, detail="Address not found")

    provider_id = address_record.provider_id
    db.delete(address_record)
    db.commit()

    remaining_addresses = db.query(Pch_Provider_Address).filter(
        Pch_Provider_Address.provider_id == provider_id
    ).all()
...
@router.get("/pch-crm-new/entity-filters")
...
@router.get("/pch-crm-new/provider-search")
...
@router.get("/pch-crm-new/providers/by-npi")
...
@router.get("/pch-crm-new/provider-locations")
...
@router.get("/pch-crm-new/provider-location-history")
...
@router.get("/pch-crm-new/provider-identifiers")
...
@router.get("/pch-crm-new/provider-identifier-history")
...
@router.get("/pch-crm-new/provider-education")
...
@router.get("/pch-crm-new/provider-education-history")
...
@router.get("/pch-crm-new/regulatory-validation")
...
@router.get("/pch-crm-new/regulatory-validation-history")
...
@router.get("/pch-crm-new/ops-logs/check-started")
...
@router.get("/pch-crm-new/affiliation")
...
@router.get("/pch-crm-new/affiliation-history")
...
@router.get("/pch-crm-new/carrier-contracts")
...
@router.post(
    "/pch-crm-new/carrier-contract", response_model=List[PchCarrierContractingSchema]
)
...
@router.get("/pch-crm-new/sales-services")
...
@router.delete("/pch-crm-new/sales")
...
@router.get("/pch-crm-new/networks")
async def get_crm_networks(
    ...
    query = db.query(Pch_Networks, Sub_Entity).outerjoin(
        Sub_Entity,
        Sub_Entity.sub_entity_id == Pch_Networks.affiliation
    )
    ...
    items = []
    for network, sub_entity in paginated["items"]:
        item_dict = dict(network.__dict__)
        item_dict.pop("_sa_instance_state", None)

        if sub_entity:
            first = sub_entity.sub_entity_fname or ""
            last = sub_entity.sub_entity_lname or ""
            item_dict["affiliation_name"] = f"{first} {last}".strip()
        else:
...
@router.post("/pch-crm-new/networks")
...
@router.patch("/pch-crm-new/networks")
...
async def get_crm_notes(
    ...
    query = db.query(Pch_Notes).filter(Pch_Notes.module == module)
...
@router.post("/pch-crm-new/notes") #, response_model=PchNotesSchema
...
@router.patch("/pch-crm-new/notes") #, response_model=PchNotesSchema
...
@router.delete("/pch-crm-new/notes")
...
@router.get("/pch-crm-new/attachements")
async def get_attachements(
    ...
    query = db.query(pch_attachments)
...
@router.post("/pch-crm-new/attachements")
...
@router.delete("/pch-crm-new/attachments")
...
@router.post(
    "/pch-crm-new/audit-history/bulk",
...
@router.get("/pch-crm-new/provider/summary")
...
@router.post("/pch-crm-new/provider")
...
@router.post("/pch-crm-new/provider/upload")
async def upload_provider(
    providers: PchProviderInfoCreateSchema | List[PchProviderInfoCreateSchema],
    module: str,
    db: Session = Depends(get_db),
    user: dict = Depends(get_current_user),
    ...
    incoming_keys = [(p.npi, p.company_id, p.group_id) for p in providers if p.npi]

    if not incoming_keys:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No valid NPI provided."
        )

    existing_stmt = select(
        Pch_Provider_Info.npi,
        Pch_Provider_Info.company_id,
        Pch_Provider_Info.group_id,
        Pch_Provider_Info.txn_id,
        Pch_Provider_Info.status,
        Pch_Provider_Info.caqh_number,
    ).where(
        tuple_(
            Pch_Provider_Info.npi,
            Pch_Provider_Info.company_id,
    ...
    for (npi, company_id, group_id), info in existing_info.items():
        if info["status"] != "Prospect":
            db.execute(
                update(Pch_Provider_Info)
                .where(
                    Pch_Provider_Info.npi == npi,
                    Pch_Provider_Info.company_id == company_id,
                    Pch_Provider_Info.group_id == group_id,
                )
                .values(status="Prospect", touch_user=user.get("email"))
            )
    ...
    new_providers = [
        ...
        for p in providers
        if (p.npi, p.company_id, p.group_id) not in existing_keys
    ]
    ...
    # Collect all providers for service calls
    all_npi_info = []

    for p in new_providers:
        all_npi_info.append((p.txn_id, p.npi, p.company_id, p.caqh_number))

    for (npi, company_id, group_id), info in existing_info.items():
        all_npi_info.append((info["txn_id"], npi, company_id, info["caqh_number"]))

    parent_txn_id = str(uuid())

    # -------- MODULE BASED SERVICE CALL -------- #

    if module in ["All", "Credable"]:

        refresh_batch = [
            (txn_id, npi, company_id) for txn_id, npi, company_id, _ in all_npi_info
        ]

        asyncio.create_task(notify_external_service_batch(parent_txn_id, refresh_batch))

    if module in ["All", "CAQH"]:

        caqh_batch = [
            (txn_id, npi, company_id, caqh_number)
            for txn_id, npi, company_id, caqh_number in all_npi_info
            if caqh_number
        ]

        for txn_id, npi, company_id, caqh_number in caqh_batch:
            asyncio.create_task(
    ...
    return {
        ...
        "total_processed": len(all_npi_info),
        "module": module,
    }


@router.post("/pch-crm-new/provider/refresh")
async def refresh_data(
    npi: str, txn_id: str, company_id: str, module: str, caqh_number: str
):
    # If the module is "ALL", notify both services
    if module == "All":
        # Fire & forget for refreshing data (doesn’t block request)
        asyncio.create_task(
            notify_external_refresh_service(npi, txn_id, company_id, module)
        )

        if caqh_number:
            # If CAQH number is provided, notify external service for CAQH
            asyncio.create_task(
                notify_external_service(npi, txn_id, company_id, "CAQH", caqh_number)
            )

    # If the module is "Credable", only notify the refresh service
    elif module == "Credable":
        asyncio.create_task(
            notify_external_refresh_service(npi, txn_id, company_id, module)
        )

    # If the module is "CAQH", only notify the external service for CAQH if caqh_number is provided
    elif module == "CAQH" and caqh_number:
        asyncio.create_task(
            notify_external_service(npi, txn_id, company_id, "CAQH", caqh_number)
        )
...
@router.get("/pch-crm-new/provider/npi_search")
async def get_npi_lookup_search(npi_term: str, db: Session = Depends(get_db)):
    ...
    ).filter(npi_registry.provider_business_mailing_address_country_code == "US")

    results = []

    if term.isdigit():
        # Search by NPI prefix
        results = base_query.filter(npi_registry.npi.like(f"{term}%")).limit(100).all()
    else:
        # Search by last name first
        last_name_results = (
            base_query.filter(
                func.lower(npi_registry.provider_last_name).like(f"{term_lower}%")
            )
            .limit(100)
            .all()
        )
        ...
        # If less than 100, search by first name (excluding duplicates)
        if len(results) < 100:
            first_name_results = (
                base_query.filter(
                    func.lower(npi_registry.provider_first_name).like(f"{term_lower}%")
                )
                .filter(
                    ~npi_registry.npi.in_([r.npi for r in results])
                )  # avoid duplicates
                .limit(100 - len(results))
                .all()
            )
            results.extend(first_name_results)

    # Map results
    npis_list = [
        {
            "id": row.npi,
            "value": f"{row.npi} - {row.provider_first_name or ''}, {row.provider_last_name or ''}".strip(),
        }
        for row in results
        if row.npi
    ]
...
@router.get("/pch-crm-new/npi-lookup", response_model=NpiRegistryLookupSchema)
...
@router.put("/pch-crm-new/assign-unassign-provider")
...
@router.get("/pch-crm-new/carriers/short")
...
@router.get("/pch-crm-new/carriers-list")
async def get_carriers_list(
    searchTerm: Optional[str] = Query(None),
    db: Session = Depends(get_db),
    user: dict = Depends(get_current_user),
):
    query = db.query(Pch_Member_Carriers)

    if searchTerm:
        like = f"%{searchTerm}%"
        query = query.filter(
            or_(
                cast(Pch_Member_Carriers.pk_id, String).ilike(like),  # UUID -> text for ILIKE
                Pch_Member_Carriers.payer_id.ilike(like),
                Pch_Member_Carriers.payer_name.ilike(like),
                Pch_Member_Carriers.payer_type.ilike(like),
            )
        )

    # Order and limit
    # query = query.order_by(Pch_Member_Carriers.payer_name.asc()).limit(50)
    query = query.order_by(Pch_Member_Carriers.payer_name.asc())
...
@router.get("/pch-crm-new/carrier/{id}", response_model=CarrierCombinedSchema)
...
@router.get("/pch-crm-new/carriers-combined")
...
@router.get("/pch-crm-new/provider-caqh-disclosures")
...
@router.get("/pch-crm-new/provider-caqh-education")
...
@router.get("/pch-crm-new/provider-caqh-hospitals")
...
@router.get("/pch-crm-new/provider-caqh-identifiers")
...
@router.get("/pch-crm-new/provider-caqh-insurance")
...
@router.get("/pch-crm-new/provider-caqh-malpractice-claims")
...
@router.get(
    "/pch-crm-new/provider-caqh-practices/dropdown",
...
@router.get(
    "/pch-crm-new/provider-caqh-practices",
    summary="Get provider CAQH practices",
    description="Returns full CAQH practice records filtered by provider_id or npi, with pagination and sorting"
)
def get_provider_caqh_practices(
    provider_id: Optional[str] = Query(None, description="Filter by txn_id_provider"),
    npi: Optional[str] = Query(None, description="Filter by NPI"),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    sortColumn: str = Query("practice_name"),
    ...
    # Clean practice_name to handle empty/NULL names consistently
    name_clean = func.nullif(func.trim(Pch_Caqh_Practice.practice_name), '')

    query = db.query(Pch_Caqh_Practice).filter(name_clean.is_not(None))
    ...
    # Determine sorting
    sort_col_attr = getattr(Pch_Caqh_Practice, sortColumn, None)
    if not sort_col_attr:
        sort_col_attr = Pch_Caqh_Practice.updated_on
...
@router.get(
    "/pch-crm-new/provider-caqh-practices/{practice_uid}",
...
@router.get("/pch-crm-new/provider-caqh-provider-associates")
...
@router.get(
    "/pch-crm-new/provider-caqh-provider-info",
...
@router.get("/pch-crm-new/provider-caqh-references")
...
@router.get("/pch-crm-new/provider-caqh-specialties")
...
@router.get("/pch-crm-new/provider-caqh-work-history")
...
@router.get("/pch-crm-new/provider-communication-types/email")
...
@router.get("/pch-crm-new/provider-communication-types/phone")
...
@router.get("/pch-crm-new/provider/{provider_id}/emails")
...
@router.post("/pch-crm-new/provider/{provider_id}/emails")
...
@router.patch("/pch-crm-new/provider/{provider_id}/emails")
...
@router.delete("/pch-crm-new/provider-emails/{pk_id}")
...
@router.get("/pch-crm-new/provider/{provider_id}/phone-text")
...
@router.post("/pch-crm-new/provider/{provider_id}/phone-text")
...
@router.patch("/pch-crm-new/provider/{provider_id}/phone-text")
...
@router.delete("/pch-crm-new/provider-phone-text/{phone_id}")
...
@router.get("/pch-crm-new/provider-carrier-credentials/{txn_id_provider}")
...
@router.post("/pch-crm-new/provider-carrier-credentials")
...
@router.patch("/pch-crm-new/provider-carrier-credentials")
...
@router.delete("/pch-crm-new/provider-carrier-credentials/{pk_id}")
...
@router.get("/pch-crm-new/npi-registry-state-filter")
async def get_npi_state_filters(
    entity_id: str, sub_entity_id: str, db: Session = Depends(get_db)
):
    states = (
        db.query(npi_registry.provider_business_practice_location_address_state_name)
        .filter(
            # npi_registry.entity_id == entity_id,
            # npi_registry.subentity_id == sub_entity_id,
            npi_registry.provider_business_mailing_address_country_code == "US",
            npi_registry.provider_business_practice_location_address_state_name.isnot(
                None
            ),
        )
        .distinct()
        .order_by(npi_registry.provider_business_practice_location_address_state_name)
        .all()
    )
    ...
    for (state,) in states:
        ...
        if state_clean in VALID_US_STATES:
            valid_states.append({"id": state_clean, "value": state_clean})
        else:
            has_other = True
    ...
    if has_other:
        valid_states.append({"id": "OTHER", "value": "Other"})

    return valid_states


@router.get("/pch-crm-new/npi-registry-supporting-filters")
async def get_npi_registry_supporting_filters(
    entityId: str = Query(..., alias="entity_id"),
    subEntityId: str = Query(..., alias="sub_entity_id"),
    state: Optional[str] = Query(None, alias="state"),
    db: Session = Depends(get_db),
    ...
    try:
        ...
        base_filters = [
            npi_registry.provider_business_mailing_address_country_code == "US",
            # npi_registry.entity_id == entityId,
            # npi_registry.subentity_id == subEntityId
        ]
        ...
        if state_filter:
            ...
            if state_filter == "OTHER":
                base_filters.append(~state_column.in_(VALID_US_STATES))
            else:
                base_filters.append(state_column == state_filter)

        # Clean + extract first 5 digits
        zip_5 = func.substring(
            npi_registry.provider_business_practice_location_address_postal_code, 1, 5
        ).label("zip_5")
        ...
        query = (
            ...
            .filter(
                *base_filters,
                npi_registry.provider_business_practice_location_address_postal_code.isnot(
                    None
                ),
                func.length(
                    npi_registry.provider_business_practice_location_address_postal_code
        ...
        zips_list = [{"id": z, "value": z} for (z,) in zips if z]

        return {"zips": zips_list}

    except Exception:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch npi registry supporting filters",
        )
    
@router.get("/pch-crm-new/providers/by-npi-all")
async def get_providers_by_npi_registry(
    npi: List[str] = Query(..., description="List of NPI numbers to search"),
    db: Session = Depends(get_db),
):
    # Clean input: only valid numeric NPIs
    clean_npi = [x for x in npi if x and x.isdigit()]
    if not clean_npi:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No valid NPI provided."
    ...
    # -------------------------
    # Step 1: Look in Pch_Provider_Info
    # -------------------------
    stmt_info = select(Pch_Provider_Info).where(Pch_Provider_Info.npi.in_(clean_npi))
    provider_info_results = db.execute(stmt_info).scalars().all()

    # Map results and track found NPIs
    found_npis = set()
    mapped_results = []
    for row in provider_info_results:
        mapped_results.append(
            {
                ...
                "primary_address_1": row.primary_address_1,
                "primary_address_2": row.primary_address_2,
                "city": row.city,
                "state": row.state,
                "zip": row.zip,
            }
        )
        found_npis.add(row.npi)

    # -------------------------
    # Step 2: Query missing NPIs from NPI Registry
    # -------------------------
    missing_npis = [x for x in clean_npi if x not in found_npis]
    if missing_npis:
        # Query NPI Registry table for missing NPIs
        stmt_registry = select(
            npi_registry.npi,
            npi_registry.entity_type_code,
            npi_registry.provider_last_name,
            npi_registry.provider_first_name,
            npi_registry.provider_organization_name,
            npi_registry.provider_first_line_business_practice_location_address,
            npi_registry.provider_second_line_business_practice_location_address,
            npi_registry.provider_business_practice_location_address_city_name,
            npi_registry.provider_business_practice_location_address_state_name,
            npi_registry.provider_business_practice_location_address_postal_code,
            npi_registry.provider_gender_code,
            npi_registry.healthcare_provider_taxonomy_code_1,
            npi_registry.healthcare_provider_taxonomy_code_2,
            npi_registry.provider_credential_text,
        ).where(
            npi_registry.npi.in_(missing_npis),
            npi_registry.provider_business_mailing_address_country_code == "US",
        )

        registry_results = db.execute(stmt_registry).all()
            ...
            mapped_results.append(
                {
                    ...
                    "last_name": (
                        ...
                        if str(row.entity_type_code) == "1"
                        else row.provider_organization_name
                    ),
                    "first_name": row.provider_first_name,
                    "primary_address_1": row.provider_first_line_business_practice_location_address,
                    "primary_address_2": row.provider_second_line_business_practice_location_address,
...
@router.get("/pch-crm-new/npi-registry-providers")
async def get_npi_registry_providers(
    entity_id: str,
    sub_entity_id: str,
    ...
    filters = [
        # npi_registry.entity_id == entity_id,
        # npi_registry.subentity_id == sub_entity_id,
        npi_registry.provider_business_mailing_address_country_code
        == "US",
    ]

    if state:
        # filters.append(
        #     npi_registry.provider_business_practice_location_address_state_name == state
        # )
        if state:
            state_clean = state.strip().upper()

            state_column = func.upper(
                func.trim(
                    npi_registry.provider_business_practice_location_address_state_name
                )
            )

            if state_clean == "OTHER":
                filters.append(not_(state_column.in_(VALID_US_STATES)))
            else:
                filters.append(state_column == state_clean)

    if npi:
        clean_npi = [x for x in npi if x]
        if clean_npi:
            filters.append(npi_registry.npi.in_(clean_npi))

    if zip:
        clean_zip = [x for x in zip if x]
        if clean_zip:
            zip_conditions = [
                npi_registry.provider_business_practice_location_address_postal_code.like(
                    f"{z}%"
                )  # starts with
                for z in clean_zip
            ]
    ...
    # Sorting
    if sort_column and hasattr(npi_registry, sort_column):
        sort_attr = getattr(npi_registry, sort_column)
        stmt = stmt.order_by(
            desc(sort_attr) if sort_order.lower() == "desc" else asc(sort_attr)
        )
    else:
        stmt = stmt.order_by(npi_registry.npi.asc())

    total_count = db.execute(
        select(func.count()).select_from(npi_registry).where(and_(*filters))
    ).scalar()

    stmt = stmt.offset((page - 1) * page_size).limit(page_size)

    results = db.execute(stmt).all()  # .scalars()

    # Map to Pch_Provider_Info structure
    mapped_results = []
    for row in results:
        mapped_results.append(
            {
                "status": None,
                "type": (
                    "Individual" if str(row.entity_type_code) == "1" else "Organization"
                ),
                "npi": row.npi,
                "last_name": (
                    row.provider_last_name
                ...
                "race": None,
                "email": None,
                "primary_speciality": row.healthcare_provider_taxonomy_code_1,
                "secondary_speciality": row.healthcare_provider_taxonomy_code_2,
                "professional_degree": row.provider_credential_text,
...
@router.get("/pch-crm-new/npi-registry-search")
async def search_npi_registry(
    ...
    try:
        ...
        # --- Search logic ---
        if npiTerm.isdigit():
            query = query.filter(npi_registry.npi.like(f"{npiTerm}%"))
        else:
            term_lower = npiTerm.strip().lower()
            ...
            if len(parts) == 2:
                ...
                query = query.filter(
                    or_(
                        and_(
                            func.lower(npi_registry.provider_first_name).like(
                                f"{first_part}%"
                            ),
                            func.lower(npi_registry.provider_last_name).like(
                                f"{second_part}%"
                            ),
                        ),
                        and_(
                ...
                query = query.filter(
                    or_(
                        ...
                        func.lower(npi_registry.provider_organization_name).like(
                            f"{term_lower}%"
                        ),  # new partial index
                    )
                )
                # query = query.filter(
                #     or_(
                #         npi_registry.provider_first_name.ilike(f"{term}%"),
                #         npi_registry.provider_last_name.ilike(f"{term}%"),
                #         npi_registry.npi.ilike(f"%{term}%"),
                #     )
                # )

        # --- Filters ---
        if entity_id:
            query = query.filter(npi_registry.entity_id == entity_id)

        if sub_entity_id:
            query = query.filter(npi_registry.subentity_id == sub_entity_id)
        ...
        if zip:
            clean_zip = [z for z in zip if z]
            if clean_zip:
                zip_conditions = [
                    npi_registry.provider_business_practice_location_address_postal_code.ilike(
                        f"{z}%"
                    )
                    for z in clean_zip
                ]
                query = query.filter(or_(*zip_conditions))

        # --- Ordering + Pagination ---
        query = (
            query.order_by(
                npi_registry.provider_last_name, npi_registry.provider_first_name
            )
            .limit(limit)
            .offset(offset)
        )
...
@router.get("/pch-crm-new/npi-registry-details", response_model=PchProviderInfoSchema)
async def get_npi_registry_provider_details(
    entity_id: str,
    sub_entity_id: str,
    npi: str,
    db: Session = Depends(get_db),
):
    print("/pch-crm-new/npi-registry-details")
    stmt = db.query(
        npi_registry.npi,
        npi_registry.entity_type_code,
        npi_registry.provider_last_name,
    ...
    mapped_response = PchProviderInfoSchema(
        status="Lead",
        type="Individual" if str(query.entity_type_code) == "1" else "Organization",
        npi=query.npi,
        last_name=query.provider_last_name,
        first_name=query.provider_first_name,
        primary_address_1=query.provider_first_line_business_practice_location_address,
        primary_address_2=query.provider_second_line_business_practice_location_address,
        city=query.provider_business_practice_location_address_city_name,
        state=query.provider_business_practice_location_address_state_name,
        zip=query.provider_business_practice_location_address_postal_code,
        gender=query.provider_gender_code,
        race=None,
        email=None,
        primary_speciality=query.healthcare_provider_taxonomy_code_1,
        secondary_speciality=query.healthcare_provider_taxonomy_code_2,
        professional_degree=query.provider_credential_text,
        rx_waiver_expiration_date=None,
        awards=None,
        language=None,
        board_cert=None,
...
@router.get("/pch-crm-new/npi-registry-addresses/{npi}")
def get_npi_addresses(npi: str, db: Session = Depends(get_db)):

    record = (
        db.query(
            # Practice Address
            npi_registry.provider_first_line_business_practice_location_address.label(
                "line1"
            ),
            npi_registry.provider_second_line_business_practice_location_address.label(
                "line2"
            ),
            npi_registry.provider_business_practice_location_address_city_name.label(
                "city"
            ),
            npi_registry.provider_business_practice_location_address_state_name.label(
                "state"
            ),
            npi_registry.provider_business_practice_location_address_postal_code.label(
                "zip"
            ),
            # Mailing Address
            npi_registry.provider_first_line_business_mailing_address.label(
                "mail_line1"
            ),
            npi_registry.provider_second_line_business_mailing_address.label(
                "mail_line2"
            ),
            npi_registry.provider_business_mailing_address_city_name.label("mail_city"),
            npi_registry.provider_business_mailing_address_state_name.label(
                "mail_state"
            ),
            npi_registry.provider_business_mailing_address_postal_code.label(
                "mail_zip"
            ),
        )
        .filter(npi_registry.npi == npi)
        .first()
    )
    ...
    # Mailing Address
    if record.mail_line1:
        addresses.append(
            {
                "provider_npi": npi,
                "address_type": "Mailing",
                "address_type_id": "Mailing",
                "line1": record.mail_line1,
                "line2": record.mail_line2,
                "street": "",
...
@router.get("/pch-crm-new/npi-registry-provider/{npi}/emails")
def get_npi_emails(npi: str, db: Session = Depends(get_db)):
...
@router.get("/pch-crm-new/npi-registry-provider/{npi}/phone-text")
def get_npi_phone_text(npi: str, db: Session = Depends(get_db)):

    # Step 1: Fetch NPI Registry record
    record = (
        db.query(
            npi_registry.provider_business_practice_location_address_telephone_number.label(
                "practice_tel"
            ),
            npi_registry.provider_business_practice_location_address_fax_number.label(
                "practice_fax"
            ),
            npi_registry.provider_business_mailing_address_telephone_number.label(
                "mail_tel"
            ),
            npi_registry.provider_business_mailing_address_fax_number.label("mail_fax"),
        )
        .filter(npi_registry.npi == npi)
        .first()
    )
    ...
    # Step 2: Collect phone/fax numbers, avoiding duplicates
    numbers = []

    # Practice Telephone
    if record.practice_tel:
        numbers.append(
            {
                "comm_type": "phone",
                "communication_type": "phone",
                "type": "primary",
                "value": record.practice_tel,
                "phone": record.practice_tel,
                "primary": True,
            }
        )

    # Practice Fax — only if different from telephone
    # if record.practice_fax and record.practice_fax != record.practice_tel:
    #     numbers.append({
    #         "type": "Practice Fax",
    #         "value": record.practice_fax
    #     })

    # Mailing Telephone — skip if same as practice telephone
    if record.mail_tel and record.mail_tel not in [
        record.practice_tel,
        record.practice_fax,
    ]:
        numbers.append(
            {
                ...
                "communication_type": "phone",
                "type": "cell",
                "value": record.mail_tel,
                "phone": record.mail_tel,
                "primary": False,
            }
        )

    # Mailing Fax — skip if same as any previous
    # if record.mail_fax and record.mail_fax not in [record.practice_tel, record.practice_fax, record.mail_tel]:
    #     numbers.append({
    #         "type": "Mailing Fax",
    #         "value": record.mail_fax
    #     })
