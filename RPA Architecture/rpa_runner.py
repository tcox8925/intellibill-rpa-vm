...
def get_target_dates(target_date_str, cadence):
    if cadence.lower() == "daily":
        ...
        iso_year = today.isocalendar()[0]  # Dynamic ISO week-year from 'today'
        target_dates = []
        # 1 = Monday, 7 = Sunday
        for d in weekdays:
            target_dates.append(datetime.fromisocalendar(iso_year, current_week_number, d).date())
        return target_dates
...
def run_all_carriers():
    ...
    for _, row in rpa_matrix.iterrows():
        ...
        try:
            ...
            # if carrier_name != 'Zing Health' or process_name != 'BOB':
...
try:
    ...
    resp = requests.post(
        "http://localhost:8000/service_interruptions/run",
        timeout=30
    )
