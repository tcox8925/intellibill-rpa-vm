...
def get_contracts_by_carrier(carrier_id, status_str, use_test_npns, batch_size=15, appointment_type_limit=None, agent_type_limit=None):
    ...
    if use_test_npns == 'Yes':
        ...
        query_filter = f"""
                WHERE
                (carrier_id = '{carrier_id}') AND (contract.npn = '17765163')
            """
    else:
        ...
        query_filter = f"""
                ...
                AND contract.npn != '101000117')\n
            """

    query = f"""SELECT contract_id_crm as id
                        ,carrier_id
                        ,agent_id_crm
    ...
    if carrier_id == '2931751000020024159':
        print("==Ambetter detected; Performing requested_state reformat...")
        for i, contract in df.iterrows():
            state_str = ''
            if contract['requested_state'] is not None:
                for k, state in enumerate(contract['requested_state']):
...
def bcbs_mi_firm_adjustments(carrier_id, status_str, firm_contracts):
    ...
    for i, contract in firm_contracts.iterrows():
        ...
        try:
            ...
            try:
                ...
                query = f"""
                        SELECT *
                        FROM wpo.lup_agents
                        WHERE responsible_agency = '{contract['agent_id']}'
                        """
                # Load data into a Pandas DataFrame
        ...
        if contract_df is None:
            ...
            trimmed_df['contract.status'] = 'Requested'
    ...
    for i, contract in firm_contracts.iterrows():
        ...
        try:
            ...
            if contract['last_name']:
                contract['agency_full_name'] = contract['first_name'] + " " + contract['last_name']
            else:
                contract['agency_full_name'] = contract['first_name']
...
def uhc_aca_subproducer_adjustments(carrier_id, status_str, subproducer_contracts):
    ...
    for i, contract in subproducer_contracts.iterrows():
        ...
        if contract_df is None:
            ...
            contract['parent_full_name'] = contract_df['first_name'].item() + " " + contract_df['last_name'].item()
            contract['parent_first_name'] = contract_df['first_name'].item()
            contract['parent_last_name'] = contract_df['last_name'].item()
    ...
    for i, contract in firm_contracts.iterrows():
        ...
        try:
            ...
            if len(agent_df) == 0:
                ...
                print("==More than one responsible agent was found, choosing first.")
                agent_df = agent_df.iloc[:1]
            agent_id = agent_df['agent_id_crm'].item()
            contract['responsible_agent'] = agent_id
