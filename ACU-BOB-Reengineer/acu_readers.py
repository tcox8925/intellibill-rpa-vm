...
def read_quartz(blob_service_client, blob_name, rule, column_mappings,
    ...
    if not aca_df.empty:
        out = pd.DataFrame()
        out["agent_npn"] = aca_df.get(col.get("AGENT NPN", ""), "")
        out["agent_writing_num"] = out["agent_npn"]
        out["agent_fname"] = aca_df.get(col.get("AGENT FIRST NAME", ""), "")
        out["agent_lname"] = aca_df.get(col.get("AGENT LAST NAME", ""), "")
        out["appointed_state"] = ""  # ACA doesn't use MA STATE per guide
    ...
    if not mdc_df.empty:
        out = pd.DataFrame()
        out["agent_npn"] = mdc_df.get(col.get("AGENT NPN", ""), "")
        out["agent_writing_num"] = out["agent_npn"]
        out["agent_fname"] = mdc_df.get(col.get("AGENT FIRST NAME", ""), "")
        out["agent_lname"] = mdc_df.get(col.get("AGENT LAST NAME", ""), "")
        out["appointed_state"] = mdc_df.get(col.get("MA STATE", ""), "").fillna("")
        out["contract_status"] = "Active"
...
def read_christus(blob_service_client, blob_name, rule, column_mappings,
    ...
    df["agent_npn"] = df["NPN"].fillna("").str.strip()
    df["agent_writing_num"] = df["agent_npn"]
    df["agent_fname"] = df["First Name"].fillna("")
    df["agent_lname"] = df["Last Name"].fillna("")
    df["appointed_state"] = df["State"].fillna("")
    df["contract_status"] = df["Broker Status"].fillna("")
    df["appointment_type"] = df["Sub Type"].fillna("")  # matrix value_map transforms this
    ...
    if suspended_mask.any():
        ...
        agent_notes = suspended.groupby("agent_npn")["_note_part"].apply(
            lambda parts: "Suspended: " + ", ".join(sorted(set(parts)))
        ).to_dict()
    ...
    if mdc_mask.any():
        ...
        results.append((_make_sub_rule(rule, "CHRISTUS - MDC", CHRISTUS_MDC_ID), mdc_df))
...
def read_community_health(blob_service_client, blob_name, rule, column_mappings,
    ...
    # Contract year logic:
    #   current year  → Active
    #   previous year → Active - Recertification Needed
    #   anything else → excluded
    current_year = pd.Timestamp.now().year
    previous_year = current_year - 1
    contract_year = df[col_contract_year].fillna("").str.strip()

    current_mask = contract_year == str(current_year)
    previous_mask = contract_year == str(previous_year)
    df = df[current_mask | previous_mask].copy()

    # Set contract_status based on year
    df["_contract_year_status"] = ""
    df.loc[current_mask[df.index], "_contract_year_status"] = "Active"
    df.loc[previous_mask[df.index], "_contract_year_status"] = "Active - Recertification Needed"
    print(f"    Filtered to years {current_year}/{previous_year}: {len(df)} rows ({current_mask.sum()} current, {previous_mask.sum()} previous)")

    # Build canonical columns
    df["agent_npn"] = df[col_agent_npn].fillna("").str.strip()
    df["agent_writing_num"] = df[col_assoc_number].fillna("").str.strip()
    df["agent_full_name"] = df[col_agent_name].fillna("")
    # contract_status: year determines the label, but only for Active associations
    assoc_status = df[col_assoc_status].fillna("").str.strip()
    df["contract_status"] = assoc_status  # default: keep original
    active_current = (assoc_status == "Active") & (df["_contract_year_status"] == "Active")
    active_previous = (assoc_status == "Active") & (df["_contract_year_status"] == "Active - Recertification Needed")
    df.loc[active_current, "contract_status"] = "Active"
    df.loc[active_previous, "contract_status"] = "Active - Recertification Needed"
...
def read_molina(blob_service_client, blob_name, rule, column_mappings,
    ...
    # Map columns
    df["agent_npn"] = df["NPN"].fillna("").str.strip() if "NPN" in df.columns else ""
    df["agent_writing_num"] = df["agent_npn"]
    df["agent_fname"] = df["First Name"].fillna("") if "First Name" in df.columns else ""
    df["agent_lname"] = df["Last Name"].fillna("") if "Last Name" in df.columns else ""
    df["appointed_state"] = df["State"].fillna("") if "State" in df.columns else ""
    ...
    if mdc_mask.any():
        ...
        print(f"      Molina-MDC: {len(mdc_df)} rows")
...
def read_healthfirst(blob_service_client, blob_name, rule, column_mappings,
    ...
    df = pd.read_excel(xls, sheet_name=data_sheet[0], dtype=str)
    df.columns = df.columns.str.strip()
    print(f"    Health First: {len(df)} rows from '{data_sheet[0]}'")
    ...
    df["agent_lname"] = df["Last Name"].fillna("")
    df["appointed_state"] = df["State"].fillna("")
    df["contract_status"] = df["Appointment Status"].fillna("")
    ...
    if aca_mask.any():
        ...
        aca_df["parent_npn"] = "15124705"  # Agility
    ...
    if mdc_mask.any():
        mdc_df = df[mdc_mask].copy()
        mdc_df["appointment_type"] = "Producer"
        mdc_df["parent_npn"] = ""
