...
# ==========================================================
def generate_run_report(all_metrics: List[Dict], scan_date: str, process_type: str = "ACU") -> AIResult:
    """
    ...
    scan_date : str
        Run date string.
    process_type : str
        "ACU" or "BOB". Labels the run for the model. BOB metrics omit the
        missing-agents stage, so missing_count is read defensively downstream.
    ...
    # ── Build prompt ──
    prompt = build_run_report_prompt(all_metrics, scan_date, process_type)
