...
def build_run_report_prompt(all_metrics: List[Dict], scan_date: str, process_type: str = "ACU") -> str:
    """
    Build one comprehensive prompt with per-carrier data.
    The model sees the full picture and writes the analysis.

    process_type ("ACU" or "BOB") labels the run in the prompt. BOB metrics
    do not carry a missing-agents stage, so missing_count is read defensively.
    """
    ...
    for m in sorted(all_metrics, key=lambda x: -x.get("exception_rate", 0)):
        ...
        carrier_lines.append(
            ...
            f"missing={m.get('missing_count', 0)}, "
    ...
    return f"""Analyze this {process_type} pipeline run and write the analysis section of the report.
