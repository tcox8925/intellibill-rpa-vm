...
def _run_totals(all_metrics):
    ...
    total_missing = sum(m.get("missing_count", 0) for m in all_metrics)
...
def build_notification(all_metrics, run_date_str, uploaded, new_carriers, deactivated,
                       ...
                       schema_drift_carriers=None, row_variance_carriers=None,
                       value_drift_carriers=None, process_type="ACU",
                       row_increase_carriers=None):
    ...
    lines = []
    lines.append(f"{process_type} Processing Report{mode}")
    ...
    if schema_drift_carriers:
        lines.append(f"\nSCHEMA DRIFT DETECTED ({len(schema_drift_carriers)}):")
        for d in schema_drift_carriers:
            if d["type"] == "critical":
                lines.append(f"  🚫 {d['carrier_name']} — CRITICAL (deactivated)")
                lines.append(f"     Removed mapped columns: {', '.join(d['removed'])}")
                if d.get("added"):
                    lines.append(f"     New columns: {', '.join(d['added'])}")
                lines.append(f"     Action: {d['action']}")
            elif d["type"] == "soft":
                lines.append(f"  ℹ️  {d['carrier_name']} — New columns added")
                lines.append(f"     New columns: {', '.join(d['new_columns'])}")
                lines.append(f"     Action: {d['action']}")

    if value_drift_carriers:
        lines.append(f"\nVALUE DRIFT DETECTED ({len(value_drift_carriers)}):")
        for v in value_drift_carriers:
            severity = "CRITICAL (deactivated)" if v.get("type") == "critical" else "Soft"
            icon = "🚫" if v.get("type") == "critical" else "ℹ️ "
            lines.append(f"  {icon} {v['carrier_name']} — {severity}")
            for col_warn in v.get("columns", []):
                lines.append(f"     {col_warn['column']}: {col_warn['detail']}")
                if col_warn.get("prev_samples"):
                    lines.append(f"       was: {col_warn['prev_samples']}")
                if col_warn.get("curr_samples"):
                    lines.append(f"       now: {col_warn['curr_samples']}")
            lines.append(f"     Action: {v.get('action', '')}")

    if row_variance_carriers:
        lines.append(f"\nROW VARIANCE ALERTS ({len(row_variance_carriers)}):")
        for v in row_variance_carriers:
            lines.append(f"  🔴 {v['carrier_name']}: {v['previous']:,} → {v['current']:,} ({v['variance_pct']}% change)")
            lines.append(f"     Action: {v['action']}")

    if row_increase_carriers:
        lines.append(f"\nROW INCREASE (FYI — still processed) ({len(row_increase_carriers)}):")
        for v in row_increase_carriers:
            lines.append(f"  🔵 {v['carrier_name']}: {v['previous']:,} → {v['current']:,} (+{v['variance_pct']}%)")
...
def build_notification_html(all_metrics, run_date_str, uploaded, new_carriers, deactivated,
    ...
    <div style="border-bottom: 3px solid #2b579a; padding-bottom: 12px; margin-bottom: 16px;">
        <h2 style="margin: 0; color: #2b579a; font-size: 20px;">{esc(process_type)} Processing Report{esc(mode)}</h2>
    ...
    if schema_drift_carriers:
        ...
        for d in schema_drift_carriers:
            if d["type"] == "critical":
                sev_color = "#c4314b"
                sev_label = "CRITICAL"
                detail = f"Removed: {esc(', '.join(d['removed']))}"
                if d.get("added"):
                    detail += f"<br/>Added: {esc(', '.join(d['added']))}"
            else:
                sev_color = "#d48806"
                sev_label = "New Columns"
                detail = f"Added: {esc(', '.join(d['new_columns']))}"
            drift_rows += f"""<tr style="background: #ffffff;">
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(d['carrier_name'])}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: {sev_color}; font-weight: 600;">{sev_label}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a; font-size: 12px;">{detail}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(d['action'])}</td>
            </tr>"""
        drift_table = f"""
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; background: #ffffff;">
            <tr style="background: #f0f2f5;">
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Carrier</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Severity</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Details</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Action</th>
            </tr>
            {drift_rows}
        </table>"""
        html.append(_section(f"Schema Drift Detected ({len(schema_drift_carriers)})", "#d48806", drift_table))

    # Value Drift section
    if value_drift_carriers:
        vd_rows = ""
        for v in value_drift_carriers:
            sev_color = "#c4314b" if v.get("type") == "critical" else "#d48806"
            sev_label = "CRITICAL" if v.get("type") == "critical" else "Soft"
            col_details = "<br/>".join(
                f"{esc(w['column'])}: {esc(w['detail'])}"
                + (f"<br/><span style='color:#888;font-size:11px;'>was: {esc(str(w.get('prev_samples',[])))}</span>" if w.get("prev_samples") else "")
                + (f"<br/><span style='color:#888;font-size:11px;'>now: {esc(str(w.get('curr_samples',[])))}</span>" if w.get("curr_samples") else "")
                for w in v.get("columns", [])
            )
            vd_rows += f"""<tr style="background: #ffffff;">
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v['carrier_name'])}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: {sev_color}; font-weight: 600;">{sev_label}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a; font-size: 12px;">{col_details}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v.get('action', ''))}</td>
            </tr>"""
        vd_table = f"""
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; background: #ffffff;">
            <tr style="background: #f0f2f5;">
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Carrier</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Severity</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Columns</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Action</th>
            </tr>
            {vd_rows}
        </table>"""
        html.append(_section(f"Value Drift Detected ({len(value_drift_carriers)})", "#c4314b", vd_table))

    # Row Variance section
    if row_variance_carriers:
        var_rows = ""
        for v in row_variance_carriers:
            var_rows += f"""<tr style="background: #ffffff;">
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v['carrier_name'])}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #1a1a1a;">{v['previous']:,}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #1a1a1a;">{v['current']:,}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #c4314b; font-weight: 600;">{v['variance_pct']}%</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v['action'])}</td>
            </tr>"""
        var_table = f"""
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; background: #ffffff;">
            <tr style="background: #f0f2f5;">
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Carrier</th>
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Previous</th>
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Current</th>
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Variance</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Action</th>
            </tr>
            {var_rows}
        </table>"""
        html.append(_section(f"Row Variance Alerts ({len(row_variance_carriers)})", "#c4314b", var_table))

    # Row Increase section (informational — carrier stays active)
    if row_increase_carriers:
        inc_rows = ""
        for v in row_increase_carriers:
            inc_rows += f"""<tr style="background: #ffffff;">
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v['carrier_name'])}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #1a1a1a;">{v['previous']:,}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #1a1a1a;">{v['current']:,}</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; text-align: right; color: #0969da; font-weight: 600;">+{v['variance_pct']}%</td>
                <td style="padding: 6px 10px; border-bottom: 1px solid #eee; color: #1a1a1a;">{esc(v['action'])}</td>
            </tr>"""
        inc_table = f"""
        <table style="width: 100%; border-collapse: collapse; font-size: 13px; background: #ffffff;">
            <tr style="background: #f0f2f5;">
                ...
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Previous</th>
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Current</th>
                <th style="padding: 8px 10px; text-align: right; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Increase</th>
                <th style="padding: 8px 10px; text-align: left; font-weight: 600; color: #1a1a1a; border-bottom: 2px solid #dde1e6;">Action</th>
            </tr>
            {inc_rows}
        </table>"""
        html.append(_section(f"Row Increase (FYI — still processed) ({len(row_increase_carriers)})", "#0969da", inc_table))
    ...
    <div style="border-top: 1px solid #dde1e6; padding-top: 8px; margin-top: 20px; font-size: 11px; color: #999;">
        834 Labs - Data Operations | {esc(process_type)} Pipeline
    </div>
    </div></body></html>""")
