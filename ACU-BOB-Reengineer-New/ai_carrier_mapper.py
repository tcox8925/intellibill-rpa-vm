...
BOB_CANONICAL_COLUMNS = [
    # Agent
    "agent_npn", "agent_writing_num", "agent_full_name", "agent_fname", "agent_lname",
    "carrier_agent_name", "agent_email", "agent_start_date", "agent_end_date",
    "agent_npn_20", "agent_name20", "agent_status20", "agent_wid20",
    # Member identity
    "mem_id", "mem_policy_num", "mem_full_name", "mem_fname", "mem_lname",
    "mem_dob", "mem_age", "mem_gender", "mem_ssn",
    # Member address
    "mem_address1", "mem_address2", "mem_city", "mem_state", "mem_county", "mem_zip",
    "mem_email", "mem_phone_num", "mem_rating_zip", "mem_zip_plus4",
    "mem_second_address1", "mem_second_address2", "mem_second_city",
    "mem_second_state", "mem_second_county", "mem_second_zip", "mem_second_phone_num",
    # Member coverage
    "mem_status", "mem_market", "mem_effective_date", "mem_cov_end_date",
    "mem_enroll_date", "mem_enroll_type", "mem_app_date", "mem_paid_thru_date",
    "mem_plan_year", "mem_premium", "mem_policy_premium", "mem_count",
    # Product / plan
    "product_type", "market_type", "plan_status", "cov_type", "cov_grp", "cov_date",
    "coverage_status", "exch_on_off", "exch_mem_id",
    # Financial
    "payment_status", "bill_status", "amt_due", "policy_premium", "policy_premium_renew",
    "policy_due_date", "aptc_subsidy", "aptc_subsidy_renew", "aptc_subsidy_start_date",
    "aptc_subsidy_toggle", "aptc_mar_num",
    # Application
    "ffm_id", "ffm_app_num", "app_num", "app_recvd_date", "mar_num",
    # Hierarchy / commissions
    "parent_agent", "report_agent", "payee", "comm_assign_payee",
    "comm_eligible", "comm_eff_date",
    "ga_npn_30", "ga_status30", "mga_npn_40", "mga_status40",
    "fmo_npn_50", "fmo_status50", "nma_npn_60", "nma_status60",
    "nma_npn_70", "nma_status70", "nma_npn_80", "nma_status80",
    "solicitor_npn_10", "sol_status10",
    # Renewal
    "renew_type", "renew_plan", "renew_flag", "renew_date",
    # Other
    "link_start_date", "aging_status", "account_creation_status",
    "emp_grp_name", "emp_subsidy_type", "emp_subsidy_amt", "mem_status_uhc",
]
...
def load_existing_mappings(conn, process_type="ACU"):
    """Load existing mappings: {database_column: [list of file column names from other carriers]}.
    Filtered by process_type so BOB suggestions use BOB examples, ACU uses ACU."""
    query = f"""
    ...
    """
    df = pd.read_sql(query, conn, params=[process_type])
    examples = {}
...
RULES_FIELDS = [
    ...
    "fallback_identity_field", "default_type_value",
    "type_value_map", "rts_flag_applicable",
]
...
RULES FIELDS TO SUGGEST (provide a value for each):
  ...
  default_type_value — Producer, Subproducer, or NA
...
def _fallback_rules(file_info, headers, sample_data=None, existing_rules=None):
    ...
    if existing_rules:
        proc = file_info.get("process_type", "ACU")
        appt_types = [r.get("default_type_value", "") for r in existing_rules
                      if r.get("process_type") == proc
                      and r.get("default_type_value")
                      and str(r["default_type_value"]).strip() not in ("", "nan", "NA", "None")]
        if appt_types:
            from collections import Counter
            most_common = Counter(appt_types).most_common(1)[0][0]
            rules["default_type_value"] = most_common
...
def store_suggestions(conn, file_info, suggestions, rule_suggestion):
    ...
    if FEATURES.get("test_mode"):
        print(f"    🧪 [test mode] skipping AI mapping suggestion insert for {file_info.get('file_name','?')}")
        return
    cur = conn.cursor()
...
def detect_new_carriers(files, known_prefixes, blob_client, conn,
    ...
    existing_mappings = load_existing_mappings(conn, process_type)
